...
export function useApi(fn, deps = [], { inicial = null } = {}) {
  ...
  useEffect(() => {
    ...
    Promise.resolve()
      .then(() => fnRef.current())
      .then((data) => { if (vivo) setEstado({ data, cargando: false, error: null }) })
      .catch((error) => { if (vivo) setEstado((e) => ({ ...e, cargando: false, error })) })
    return () => { vivo = false }
  }, [claveDeps])
  ...
  return { data: estado.data, cargando: estado.cargando, error: estado.error, recargar, setData }
}
