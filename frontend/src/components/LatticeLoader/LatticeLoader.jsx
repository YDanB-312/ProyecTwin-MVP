...
import s from './LatticeLoader.module.css'
...
// Fila de estado de carga estilo consola (ReactBits LatticeLoader, JS + CSS Modules).
export default function LatticeLoader({
  ...
  const announce =
    status === 'working'
      ? `${label}, en progreso`
      : `${status === 'done' ? doneLabel : errorLabel}`

  return (
    <span
      role="status"
      className={`${s.loader}${className ? ` ${className}` : ''}`}
      data-status={status}
      data-shape={shape}
    ...
    >
      <span className={s.grid} aria-hidden="true">
        ...
        <span className={`${s.layer} ${s.mark}`}>
          {pat.cells.map((_, i) => (
            <span key={i} className={s.cell} data-on={marks[mark].includes(i) ? '' : undefined} />
          ))}
        </span>
      </span>
      <span className={s.label} aria-hidden="true">
        <span className={s.text} data-active={status === 'working' ? '' : undefined}>
          {label}
        </span>
        <span className={s.text} data-active={status === 'done' ? '' : undefined}>
          {doneLabel}
        </span>
        <span className={s.text} data-active={status === 'error' ? '' : undefined}>
          {errorLabel}
        </span>
      </span>
      {showTimer ? (
        <span ref={timerRef} className={s.timer} aria-hidden="true">
          0.0s
        </span>
      ) : null}
      <span className={s.sr}>{announce}</span>
    </span>
  )
}
